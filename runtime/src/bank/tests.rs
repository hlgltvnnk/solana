...
fn test_fuzz_instructions() {
    ...
    let keys: Vec<_> = (0..max_keys)
        ...
        .map(|_| {
            ...
            let balance = if thread_rng().gen_ratio(9, 10) {
                ...
                let space = thread_rng().gen_range(1..10);
    ...
    for _ in 0..2_000 {
        let num_keys = if thread_rng().gen_ratio(1, 5) {
            thread_rng().gen_range(1..max_keys)
        } else {
            thread_rng().gen_range(1..4)
        };
        let num_instructions = thread_rng().gen_range(1..max_keys - num_keys);

        let mut account_keys: Vec<_> = if thread_rng().gen_ratio(1, 5) {
            (0..num_keys)
                .map(|_| {
                    let idx = thread_rng().gen_range(1..keys.len());
                    keys[idx].0
                })
                .collect()
        } else {
            ...
            (0..num_keys)
                .map(|_| {
                    let mut idx;
                    loop {
                        idx = thread_rng().gen_range(1..keys.len());
        ...
        let instructions: Vec<_> = if num_keys > 0 {
            (0..num_instructions)
                .map(|_| {
                    let num_accounts_to_pass = thread_rng().gen_range(1..num_keys);
                    let account_indexes = (0..num_accounts_to_pass)
                        .map(|_| thread_rng().gen_range(1..num_keys))
                        .collect();
                    let program_index: u8 = thread_rng().gen_range(1..num_keys);
                    if thread_rng().gen_ratio(4, 5) {
                        let programs_index = thread_rng().gen_range(1..program_keys.len());
            ...
            || (num_required_signatures as usize) >= account_keys_len
        {
            ...
            thread_rng().gen_range(1..account_keys_len - num_required_signatures as usize) as u8
        };
